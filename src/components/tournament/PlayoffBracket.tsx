      ...
      <div key={match.id} className={`rounded-xl border p-3 space-y-2 w-full transition-colors relative z-10 shadow-sm ${
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="text-xs"><span className="block">{icon}{label}</span></Badge>
            <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold text-white ${getFieldColorClass(match.field)}`}>
              <span className="block">Hř. {match.field}</span>
            </span>
            {match.scheduledTime && (
              <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                <Clock className="h-3 w-3 block" />
                <span className="block">{match.scheduledTime}</span>
              </span>
            )}
          </div>
          <div className="flex items-center gap-1">
            {match.played && <Badge className="text-xs bg-primary/10 text-primary border-0"><span className="block">Dokončeno</span></Badge>}
          </div>
        </div>
        ...
        <div className="space-y-1">
          <div className={`flex items-center justify-between rounded-lg p-2 ${winnerId === match.homeTeamId ? 'bg-primary/10 font-bold' : 'bg-muted/50'}`}>
            <span className="text-sm truncate mr-2 block">{homeName}</span>
              ...
              <div className="flex items-center gap-1 shrink-0">
                {isAdmin && onUpdateMatchScore && (
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => quickGoal(match.id, 'home', -1)}>
                    <Minus className="h-3 w-3 block" />
                  </Button>
                )}
                <span className="text-sm font-bold w-4 text-center block">{match.homeScore}</span>
              </div>
            )}
            {match.played && match.homeScore !== null && <span className="text-sm font-bold w-4 text-center shrink-0 block">{match.homeScore}</span>}
          </div>
          <div className={`flex items-center justify-between rounded-lg p-2 ${winnerId === match.awayTeamId ? 'bg-primary/10 font-bold' : 'bg-muted/50'}`}>
            <span className="text-sm truncate mr-2 block">{awayName}</span>
            {isLive && !match.played && (match.awayScore !== null) && (
              <div className="flex items-center gap-1 shrink-0">
                ...
                <span className="text-sm font-bold w-4 text-center block">{match.awayScore}</span>
              </div>
            )}
            {match.played && match.awayScore !== null && <span className="text-sm font-bold w-4 text-center shrink-0 block">{match.awayScore}</span>}
          </div>
        </div>
